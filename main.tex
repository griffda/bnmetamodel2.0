...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{\small Graphical representation of the configured Bayesian Network where nodes represent the input variables (Major Radius ($R$), Aspect Ratio ($A$), Effective Ion Charge ($Z_{\text{eff}}$), Toroidal Field on Plasma ($B_{\text{T}}$)) and the output variables (High Grade Wasteheat ($H$), Net Electrical Output ($E$), and Capital Cost ($C$)) to the analytical model.}\label{fig:BN2} 
    \vspace{-15pt}
\end{figure}
